...
/**
 * classe para teste unitarios web, o service sera Mockado.
 * 
 *
 */

@SpringBootTest // para carregar o contexto
@AutoConfigureMockMvc // para não carregar o servidor TONCAT
...
//--------Configuração dos objetos Mockados ----
		...
		when(service.update(eq(nonExistingId), any())).thenThrow(ResourseNotFoundException.class);

		doNothing().when(service).delete(existingId);
		doThrow(ResourseNotFoundException.class).when(service).delete(nonExistingId);
		doThrow(DatabaseException.class).when(service).delete(depentendId);
	}
	...
	public void updateShouldReturnProductDTOWhenIdExist() throws Exception {
		...
		String expectdName = newProductDTO.getName();
		Double expectdPrice = newProductDTO.getPrice();		
		...
		result.andExpect(jsonPath("$.price").value(expectdPrice));				
	}
	
	//teste com uma rota protegida, usando o token da função
	@Test
	...
	public void findAllShouldReturnPage() throws Exception {
		...
		// checa se esta retornando algum conteudo no JSON na resposta do body
	...
	public void findbyIdShouldReturnProductWhenIdExists() throws Exception {
		...
		// checa se esta retornando id contido no JSON na resposta do body
		result.andExpect(jsonPath("$.id").exists());
		// checa se o id contido no JSON na resposta do body é igual a valor
		result.andExpect(jsonPath("$.id").value(existingId));
	}
	...
	// metodo que faz um login, usando as variaveis de test do h2 para obter um token
